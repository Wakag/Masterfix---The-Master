...
<body>
    ...
    <div class="navbar" id="sideNavbar">
        ...
        <a href="../Maintenance Fault Reports/NewPage/NewPage.php" class="nav-link">Stats for Geeks</a>
        <footer>
    ...
    <section class="top">
        ...
        $pending_tasks_sql = "SELECT COUNT(*) AS count FROM maintenancetickets WHERE Status = 'Requisition'";
        ...
        $upcoming_tasks_sql = "SELECT COUNT(*) AS count FROM maintenancetickets WHERE Status = 'Confirmed'";
        ...
        $sql = "
            ...
            WHERE maintenancetickets.Status = 'Requisition' AND 1=1 $priority_condition $category_condition $search_condition
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['TicketID']) && isset($_POST['action']) && $_POST['action'] == 'resolve') {
        $ticket_id = intval($_POST['TicketID']);  // Sanitize the input
        $update_sql = "UPDATE maintenancetickets SET Status = 'Resolved',DateResolved= NOW() WHERE TicketID = $ticket_id";
        ...
        if ($conn->query($update_sql) === TRUE) {
            ...
            <script> alert('Ticket marked as completed successfully.'); </script>
        </div>";
        } else {
