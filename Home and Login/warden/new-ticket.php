...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
    ...
    // Assuming these session variables are correctly set
    $studentId = $_SESSION['warden_id'];
    $residenceId = $_SESSION['residence_id'];
    $status = 'Confirmed';

    // Check if session variables are set
    if (empty($studentId) || empty($residenceId)) {
        die("Error: Missing student or residence ID in session.");
    }
    ...
    if (!empty($_FILES['upload']['name'])) {
        ...
        if (in_array($fileType, $allowedTypes)) {
            if (move_uploaded_file($_FILES["upload"]["tmp_name"], $targetFilePath)) {
                ...
                if ($stmt->execute()) {
                    ...
                    if ($stmt->execute()) {
                        $message = "Ticket and picture submitted successfully!";
                    } else {
                        $message = "Failed to insert picture: " . $stmt->error;
                    }
                } else {
                    $message = "Failed to insert ticket: " . $stmt->error;
                }
            } else {
        ...
        // Insert the ticket without a picture
        $stmt = $mysqli->prepare("INSERT INTO maintenancetickets (Status, DateCreated, TicketStudentID, TicketCategory, Description, TicketResID, TicketTitle) 
                                  VALUES (?, CURDATE(), ?, ?, ?, ?, ?)");
        if (!$stmt) {
...
?>
    ...
    <div class="navbar" id="sideNavbar">
        ...
        <a href="#" class="nav-link">Make a requisition</a>
